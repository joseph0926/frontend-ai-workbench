...
Frontend AI Workbench는 AI를 활용한 프론트엔드 개발을 재현 가능한 의사결정 과정으로 만드는 저장소입니다. 문제와 제약을 먼저 고정하고, 같은 조건에서 UI 후보를 비교한 뒤, 실제 브라우저의 상태·반응형·접근성 근거로 결과를 검증합니다. 구현 중 얻은 판단은 Knowledge Base와 재사용 가능한 Skill로 되돌립니다.
...
현재 `content-discovery` experiment는 하나의 콘텐츠 집합을 세 가지 탐색 모델로 제공합니다.
